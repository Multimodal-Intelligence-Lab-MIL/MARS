...
def classify_multimodal_content_single_turn(video_key, text, api_key, video_folder, turn, previous_results=None,
                                            num_frames=16):
    ...
    for attempt in range(max_retries):
        try:
            # Modification: Utilise Alibaba Cloud Baolian API calls
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    print(f"[Parse Error] Unable to parse the response: {parse_err}")
            ...
            if "data_inspection_failed" in error_message:
                print(f"[Data verification failed] Sample {video_key}, round {turn}, flagged as hateful content")
                return {"error": "data_inspection_failed", "assumed_hate": True}


            elif "insufficient_quota" in error_message or "429" in error_message or "limit" in error_message.lower():
                wait_time = 2 ** attempt
    ...
    if last_response is None:
        print(f"[Failure] No response received. Sample: {video_key}, Turn: {turn}")
        return {"error": "no_response", "error_message": "No response has been received."}

    try:

        if last_response.status_code == 200:
            result_text = last_response.output.choices[0].message.content[0]['text']
            return result_text
        else:

            print(f"[Non-standard response] Status code: {last_response.status_code}, Attempting to extract content")
            ...
            if hasattr(last_response, 'output') and last_response.output:
                try:
                    if hasattr(last_response.output, 'choices') and last_response.output.choices:
                        extracted_content = last_response.output.choices[0].message.content[0]['text']
                    else:
                        extracted_content = str(last_response.output)
                except:
                    extracted_content = str(last_response.output)

            elif hasattr(last_response, 'message'):
                extracted_content = str(last_response.message)

            else:
                extracted_content = str(last_response)
        ...
        print(f"[Parsing failed] Parsing response failed: {e}")
        print(f"[Original content] {last_response}")
...
def process_video_multi_turn(video_key, video_folder, transcript, api_key, num_frames=6):
    ...
    for turn in range(1, 5):
        ...
        raw_response = classify_multimodal_content_single_turn(
            video_key, transcript, api_key, video_folder, turn, turn_results, num_frames
        )
        ...
        if isinstance(raw_response, dict) and "error" in raw_response:
            ...
            if error_type == "data_inspection_failed":
                turn_results[f'turn_{turn}'] = {
                    'raw_response': "Data inspection failed",
                    'parsed_response': None,
                    'parse_success': False,
                    'error': 'data_inspection_failed'
                }
                print(f"[Turn {turn}] Data validation failed")
                continue
...
def process_all_videos(video_frames_dir: str, transcript_json: str, output_file: str, filter_type: str = "all",
    ...
    """
    # Modification: Set Alibaba Cloud Baolian API Key
    print("Configure Alibaba Cloud Baolian API...")
    api_key = os.getenv('DASHSCOPE_API_KEY')
    if not api_key:

        api_key = "Please replace with your actual API Key"
        print("Warning: Please set the DASHSCOPE_API_KEY environment variable or configure the API Key directly within your code.")
    ...
    for video_folder in tqdm(videos_to_process, desc=f"Processing videos with multi-turn analysis"):
        ...
        try:
            result = process_video_multi_turn(video_id, str(video_folder), transcript, api_key)
...
def main():
    ...
    output_file = "multi_turn_hate_hatemm_detection_results_llama4_all_numframe6.json"
