...
def get_frames_from_folder(folder_path, num_frames=6):
    """
    Retrieve image frames from a specified folder, consistently returning a fixed number of uniformly sampled frames.
    """
    # Retrieve all image files within the folder
    frame_files = []
    for ext in ['*.jpg', '*.jpeg', '*.png', '*.bmp']:
        frame_files.extend(glob.glob(os.path.join(folder_path, ext)))

    if not frame_files:
        print(f"Warning: In {folder_path} no images were found")
        return []

    frame_files.sort()

    # If the number of images exceeds num_frames, perform uniform sampling.
    if len(frame_files) > num_frames:
        # Indexing for uniform sampling using NumPy's linear space function
...
def create_turn_prompt(turn: int, transcript: str = "", previous_results: Dict = None) -> str:
    """
    Create prompts for different rounds, including the fourth round's comprehensive assessment
    ...
    Returns:
        Prompt for the corresponding round
    """
...
def clean_json_response(response_text: str) -> tuple:
    """Clean up model output and extract JSON content"""
    try:
...
def classify_multimodal_content_single_turn(video_key, text, model, video_folder, turn, previous_results=None,
                                            num_frames=16):
    """
    Performing single-round inference on video frames and text using Gemini 2.5 Flash
    """
    ...
    # Create the prompt for the corresponding round
    user_prompt = create_turn_prompt(turn, text, previous_results)

    # Constructing message content - Modified to Gemini format
    contents = [user_prompt]

    # Add image - Convert to Gemini format
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Check whether it is a content filtering error
...
def process_video_multi_turn(video_key, video_folder, transcript, model, num_frames=16):
    """
    Conducting multi-round dialogue analysis on individual videos - modified to four rounds
    """
    print(f"\n{'=' * 80}")
    ...
    for turn in range(1, 5):
        ...
        raw_response = classify_multimodal_content_single_turn(
            video_key, transcript, model, video_folder, turn, turn_results, num_frames
        )
        ...
        if isinstance(raw_response, dict) and "error" in raw_response:
            ...
            if error_type == "content_policy_violation":
                ...
                print(f"[Turn {turn}] Content policy violation")
                continue
...
def process_all_videos(video_frames_dir: str, transcript_json: str, output_file: str, filter_type: str = "all",
    ...
    if os.path.exists(output_file):
        try:
            ...
            # Check which videos have completed all four rounds of reasoning
            ...
            print(f"Found {len(completed_video_ids)} videos already completed with 4 turns (Gemini)")
    ...
    # Display information about the video range to be processed
    ...
    for video_folder in tqdm(videos_to_process, desc=f"Processing videos with Gemini multi-turn analysis"):
        video_id = video_folder.name
        transcript = transcripts.get(video_id, "")

        try:
            result = process_video_multi_turn(video_id, str(video_folder), transcript, model)
    ...
    _save_results(results, output_file)
    print(f"Gemini Multi-turn processing completed. Results saved to {output_file}")
...
def main():
    ...
    video_frames_dir = "/x/x/x"

    # Path to JSON file containing transcribed text
    transcript_json = "/x/x/x"

    # Output file path Output file path
    output_file = "multi_turn_hate_hatemm_detection_results_gemini25flash.json"
    ...
    print(f"Processing videos with Gemini 2.5 Flash configuration:")
