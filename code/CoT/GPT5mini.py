...
def classify_multimodal_content_cot(video_key, text, client, video_folder, num_frames=16):
...
def process_video_cot(video_key, video_folder, transcript, client, num_frames=16):
    ...
    raw_response = classify_multimodal_content_cot(
        video_key, transcript, client, video_folder, num_frames
    )
    ...
    if isinstance(raw_response, str):
        ...
        print(f"\n📝 Model Response - Unexpected response type:")
        print("🔹" + "🔹" * 25)
...
def process_all_videos(video_frames_dir: str, transcript_json: str, output_file: str, filter_type: str = "all",
    ...
    if os.path.exists(output_file):
        try:
            ...
            print(f"Found {len(completed_video_ids)} videos already completed with CoT")
    ...
    for video_folder in tqdm(videos_to_process, desc=f"Processing videos with CoT analysis"):
        video_id = video_folder.name
        transcript = transcripts.get(video_id, "")

        try:
            result = process_video_cot(video_id, str(video_folder), transcript, client)
    ...
    _save_results(results, output_file)
    print(f"CoT processing completed. Results saved to {output_file}")
...
def main():
    ...
    output_file = "cot_hate_hatemm_detection_results_gpt5mini.json"
    ...
    print(f"Processing videos with CoT configuration:")
